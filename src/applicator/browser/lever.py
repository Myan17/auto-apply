"""Playwright applicator for Lever job postings."""
    ...
    def apply(self, job: Job, docs: TailoredDocuments, confirm: bool = True) -> ApplicationResult:
        ...
        try:
            with get_browser(self.config, headless=False) as context:
                ...
                name_parts = self.config.env.applicant_name.split()
                self._fill_if_present(page, "input[name='name']", self.config.env.applicant_name)
                self._fill_if_present(page, "input[name='email']", self.config.env.applicant_email)
                self._fill_if_present(page, "input[name='phone']", "")
                self._fill_if_present(page, "input[name='org']", "")

                if docs.resume_path and Path(docs.resume_path).exists():
                    try:
                        page.set_input_files("input[type='file']", docs.resume_path, timeout=5000)
                        _random_delay(1.0)
                    except Exception:
                        pass
